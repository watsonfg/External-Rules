...
'
'  	Properties
'		Value - (Double) 			Returns current PLOAD value Of Part
...
'		 Calculate()				Determine the current PLOAD value And populates the PLOAD segment
'		 ShowPLOADFace()			Displays calculation Of PLOAD value And highlight the correct face that Is being used

Public Class PLOAD
    Private _dPLOAD As Double = 42.0
    ...
    Public iPLOAD As Inventor.Property
    ...
    Sub New(ByVal ThisDoc As Inventor.Document)
        If ThisDoc.DocumentType = Inventor.DocumentTypeEnum.kPartDocumentObject Then
            ...
            iPLOAD = iProperties.Item("PLOAD")
            If Err.Number <> 0 Then
                _errorState = True
                _strErrorValue = _strErrorValue & "\\nNo PLOAD iProperty Defined for this Part"
                Err.Clear()
            End If
    ...
    Public Sub Calculate()
        ...
        iProperties = _oDoc.PropertySets.Item("Inventor User Defined Properties")
        Dim iPLOAD As Inventor.Property

        On Error Resume Next
        iPLOAD = iProperties.Item("PLOAD")
        If Err.Number <> 0 Then
            _errorState = True
            iERROR.Value = iERROR.Value & "\\nNo PLOAD defined [I: " & _partName & "]"
        Else
            iPLOAD.Value = Round(_dPLOAD, 4)
        End If
    End Sub
